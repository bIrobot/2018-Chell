...
class RightScale(StatefulAutonomous):
    ...
    @timed_state(duration=5, next_state='right_rotate')
    ...
    @timed_state(duration=2, next_state='right_drive_backward')
    ...
    @timed_state(duration=2.8, next_state='left_cross_field')
    def left_drive_forward(self):
        self.drive.arcadeDrive(0.35, self.navx.drive(0.07, 0.2, 0), squaredInputs=False)

    @timed_state(duration=3.4, next_state='left_turn_straight')
    def left_cross_field(self):
        self.drive.arcadeDrive(0.4, self.navx.drive(0.1, 0.3, -90), squaredInputs=False)

    @timed_state(duration=2.7, next_state='left_drive_backward')
    def left_turn_straight(self):
